...
import { getWishProductsFromLocal } from "../../utilities/storeToLocal";
import CartDashboardProduct from "../CartDashboardProduct/CartDashboardProduct";
import { useNavigate } from "react-router";
import WishlistDashboardProduct from "../WishlistDashboardProduct/WishlistDashboardProduct";
...
const WishlistDashboard = () => {
  ...
  useEffect(() => {
    const localWishProductsId = getWishProductsFromLocal();

    fetch("/productsData.json")
      .then((res) => res.json())
      .then((data) => {
        const filteredProducts = data.filter((product) =>
          localWishProductsId.includes(product.product_id),
        );
  ...
  return (
    <div className="bg-gray-200 border border-blue-700">
      <div className="w-11/12 mx-auto flex items-center justify-between mt-12 mb-8">
        <h3 className="text-2xl font-bold">Wish List</h3>
